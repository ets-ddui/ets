...
#include <process/detail/windows/locale.hpp>
...
public:
    codecvt_category_t(){}
    const char* name() const BOOST_NOEXCEPT override {return "codecvt";}
...
{
    static const ::ets::process::detail::codecvt_category_t cat;
    return cat;
}
...
# if defined(BOOST_WINDOWS_API)
    ...
    return std::locale(global_loc, new ets::process::detail::windows::windows_file_codecvt);
# elif defined(macintosh) || defined(__APPLE__) || defined(__APPLE_CC__) \
|| defined(__FreeBSD__) || defined(__OpenBSD__) || defined(__HAIKU__)
    std::locale global_loc = std::locale();
...
inline std::size_t convert(const char* from,
                    ...
                    const ::ets::process::codecvt_type & cvt =
                                 ::ets::process::codecvt())
{
    ...
    if (res != std::codecvt_base::ok)
         throw process_error(res, ::ets::process::codecvt_category(),
