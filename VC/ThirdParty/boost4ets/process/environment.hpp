...
#include <process/detail/windows/environment.hpp>
#endif
...
struct const_entry
{
    ...
    typedef boost::iterator_range<pointer> range;//using range         = boost::iterator_range<pointer> ;
    typedef Environment environment_t;//using environment_t = Environment ;
    ...
    const_entry(const const_entry &p_Other)// = default;
        : _name(p_Other._name), _data(p_Other._data), _env(p_Other._env)
    {
    }
    const_entry& operator=(const const_entry &p_Other)// = default;
    {
        _name = p_Other._name;
        _data = p_Other._data;
        _env = p_Other._env;
...
{
    typedef const_entry<Char, Environment> father;//using father = const_entry<Char, Environment>;
    typedef typename father::value_type value_type;//using value_type    = typename father::value_type;
    typedef typename father::string_type string_type;//using string_type   = typename father::string_type;
    typedef typename father::pointer pointer;//using pointer       = typename father::pointer;
    typedef typename father::environment_t environment_t;//using environment_t = typename father::environment_t;
...
public:
    typedef std::basic_string<Char> string_type;//using string_type = std::basic_string<Char>;
    typedef Implementation<Char> implementation_type;//using implementation_type = Implementation<Char>;
    typedef basic_environment_impl<Char, Implementation> base_type;//using base_type = basic_environment_impl<Char, Implementation>;
    typedef detail::make_entry<Char, base_type> entry_maker;//using       entry_maker = detail::make_entry<Char, base_type>;
    typedef detail::entry <Char, base_type> entry_type;//using entry_type        = detail::entry     <Char, base_type>;
    typedef detail::const_entry <Char, const base_type> const_entry_type;//using const_entry_type  = detail::const_entry     <Char, const base_type>;
    typedef detail::make_const_entry<Char, const base_type> const_entry_maker;//using const_entry_maker = detail::make_const_entry<Char, const base_type>;

    friend       entry_type;
    friend const_entry_type;

    typedef boost::transform_iterator<entry_maker, Char**, entry_type, entry_type> iterator;//using iterator        = boost::transform_iterator<      entry_maker, Char**,       entry_type,       entry_type>;
    typedef boost::transform_iterator<const_entry_maker, Char**, const_entry_type, const_entry_type> const_iterator;//using const_iterator  = boost::transform_iterator<const_entry_maker, Char**, const_entry_type, const_entry_type>;
    ...
    {
        auto p = this->_env_impl;
        auto st1 = key + ::ets::process::detail::equal_sign<Char>();
        while (*p != nullptr)
        {
    ...
    typedef typename implementation_type::native_handle_type native_handle_type;//using native_handle_type = typename implementation_type::native_handle_type;
    using implementation_type::native_handle;

    basic_environment_impl(const detail::api::native_environment_impl<Char> &rhs) : implementation_type(rhs) {}
    basic_environment_impl() : implementation_type() {}
    basic_environment_impl(const basic_environment_impl &rhs) : implementation_type(rhs) {}
    basic_environment_impl(basic_environment_impl &&rhs) : implementation_type(std::move(rhs)) {}
    template<typename CharR>
    explicit basic_environment_impl(const basic_environment_impl<CharR, Implementation> &rhs) : implementation_type(rhs) {}

    basic_environment_impl &operator=(basic_environment_impl &&rhs)
    {
        implementation_type::operator=(std::move(rhs));
        return *this;
    }
    basic_environment_impl &operator=(const basic_environment_impl &rhs)
    {
        implementation_type::operator=(rhs);
        return *this;
    }

    template<typename CharR>
    basic_environment_impl &operator=(const basic_environment_impl<CharR, Implementation> &rhs)
    {
        implementation_type::operator=(rhs);
        return *this;
    }
...
public:
    ...
    typedef basic_environment_impl<Char, detail::api::native_environment_impl> implementation_type;
    basic_native_environment() : implementation_type() {}
    basic_native_environment(basic_native_environment &&rhs) : implementation_type(std::move(rhs)) {}

    basic_native_environment &operator=(basic_native_environment &&rhs)
    {
        implementation_type::operator=(std::move(rhs));
        return *this;
    }
...
public:
    ...
    typedef basic_environment_impl<Char, detail::api::basic_environment_impl> implementation_type;
    basic_environment(const basic_native_environment<Char> &rhs) : implementation_type(rhs) {}
    basic_environment() : implementation_type() {}
    basic_environment(const basic_environment &rhs) : implementation_type(rhs) {}
    basic_environment(basic_environment &&rhs) : implementation_type(std::move(rhs)) {}
    template<typename CharR>
    explicit basic_environment(const basic_environment<CharR> &rhs) : implementation_type(rhs) {}

    basic_environment &operator=(basic_environment &&rhs)
    {
        implementation_type::operator=(std::move(rhs));
        return *this;
    }
    basic_environment &operator=(const basic_environment &rhs)
    {
        implementation_type::operator=(rhs);
        return *this;
    }
    ...
    template<typename CharR>
    basic_environment &operator=(const basic_environment<CharR> &rhs)
    {
        implementation_type::operator=(rhs);
        return *this;
    }
...
typedef ::ets::process::detail::api::native_handle_t native_handle_type;

///Definition of the environment for this process.
using ::ets::process::native_environment;
///Definition of the environment for this process.
using ::ets::process::wnative_environment;

///Get the process id of the current process.
inline int get_id()                     { return ::ets::process::detail::api::get_id();}
///Get the native handle of the current process.
inline native_handle_type native_handle()  { return ::ets::process::detail::api::native_handle();}
///Get the enviroment of the current process.
inline native_environment   environment() { return ::ets::process:: native_environment(); }
///Get the enviroment of the current process.
inline wnative_environment wenvironment() { return ::ets::process::wnative_environment(); }
///Get the path environment variable of the current process runs.
inline std::vector<boost::filesystem::path> path()
{
#if defined(BOOST_WINDOWS_API)
    const ::ets::process::wnative_environment ne;
    //typedef typename ::ets::process::wnative_environment::const_entry_type value_type;
    ...
    auto itr = std::find_if(ne.cbegin(), ne.cend(),
            [&](const wnative_environment::const_entry_type & e)
             {return id == ::boost::to_upper_copy(e.get_name(), ::ets::process::detail::process_locale());});
