...
public:
    ...
    typedef const char_type* pointer_type;//using pointer_type = const char_type*;
    typedef std::basic_string<char_type> string_type;//using string_type = std::basic_string<char_type>;
    typedef pointer_type native_handle_type;//using native_handle_type = pointer_type;
    void reload()
    {
    ...
    native_environment_impl()// = default;
        : _buf(boost::detail::winapi::get_environment_strings<Char>(), &native_environment_impl::_deleter),
        _env_arr(_load_var(_buf.get())),
        _env_impl(&*_env_arr.begin())
    {
    }
private:native_environment_impl(const native_environment_impl& );public:// = delete;
    native_environment_impl(native_environment_impl && p_Other)// = default;
        : _buf(std::move(p_Other._buf)), _env_arr(std::move(p_Other._env_arr)), _env_impl(std::move(p_Other._env_impl))
    {
    }
private:native_environment_impl & operator=(const native_environment_impl& );public:// = delete;
    native_environment_impl & operator=(native_environment_impl && p_Other)// = default;
    {
        _buf = std::move(p_Other._buf);
        _env_arr = std::move(p_Other._env_arr);
        _env_impl = std::move(p_Other._env_impl);
    }
...
public:
    ...
    explicit inline  basic_environment_impl(
                ...
                const ::ets::process::codecvt_type & cv = ::ets::process::codecvt())
        : _data(::ets::process::detail::convert(rhs._data, cv)),
        _env_arr(_load_var(_data.data())),
        _env_impl(&*_env_arr.begin())
    {
    }
...
template<typename T> BOOST_CONSTEXPR_OR_CONST T env_seperator();
template<> BOOST_CONSTEXPR_OR_CONST  char   env_seperator() {return  ';'; }
template<> BOOST_CONSTEXPR_OR_CONST wchar_t env_seperator() {return L';'; }
