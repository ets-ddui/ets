...
#include <process/detail/windows/basic_pipe.hpp>
...
inline std::string make_pipe_name()
{
    ...
    unsigned long long pid = ::boost::detail::winapi::GetCurrentProcessId();
...
public:
    ...
    ~async_pipe()
    {
        //_sink和_source的析构函数会自动调用CloseHandle，不知boost这里为何会重复调用(会导致重复调用)
    ...
    template<class CharT, class Traits>// = std::char_traits<CharT>>
    inline operator basic_pipe<CharT, Traits>() const;
...
void async_pipe::init(const std::string & name)
{
    static BOOST_CONSTEXPR_OR_CONST int FILE_FLAG_OVERLAPPED_  = 0x40000000; //temporary
    ...
    if (source == boost::detail::winapi::INVALID_HANDLE_VALUE_)
        ::ets::process::detail::throw_last_error("create_named_pipe(" + name + ") failed");
    ...
    if (sink == ::boost::detail::winapi::INVALID_HANDLE_VALUE_)
        ::ets::process::detail::throw_last_error("create_file() failed");
