...
#include <process/detail/child_decl.hpp>
...
struct startup_info_impl
{
    ...
    startup_info_impl()
        : creation_flags(0), startup_info(startup_info_ex.StartupInfo)
    {
        memset(&startup_info_ex, 0, sizeof(startup_info_ex));
        startup_info_ex.StartupInfo.cb = sizeof(startup_info_t);
        startup_info_ex.StartupInfo.hStdInput = ::boost::detail::winapi::invalid_handle_value;
        startup_info_ex.StartupInfo.hStdOutput = ::boost::detail::winapi::invalid_handle_value;
        startup_info_ex.StartupInfo.hStdError = ::boost::detail::winapi::invalid_handle_value;
    }
...
struct startup_info_impl
{
    ...
    startup_info_t          startup_info;/*
            {sizeof(startup_info_t), nullptr, nullptr, nullptr,
             ...
             ::boost::detail::winapi::invalid_handle_value,
             ::boost::detail::winapi::invalid_handle_value};*/
    startup_info_impl() : creation_flags(0)
    {
...
class executor : public startup_info_impl<Char>
{
    ...
    typedef typename ::ets::process::detail::has_error_handler<Sequence>::type has_error_handler;
    typedef typename ::ets::process::detail::has_ignore_error <Sequence>::type has_ignore_error;

    std::error_code _ec;//{0, std::system_category()};

public:

    std::shared_ptr<boost::atomic<int>> exit_status;// = std::make_shared<std::atomic<int>>(still_active);

    executor(Sequence & seq) : seq(seq),
        _ec(0, std::system_category()),
        exit_status(std::make_shared<boost::atomic<int>>(still_active)),
        proc_attrs(nullptr), thread_attrs(nullptr), inherit_handles(false),
        work_dir(nullptr), cmd_line(nullptr), exe(nullptr), env(nullptr)
    {
    ...
    child operator()()
    {
        ...
        if (err_code != 0)
        {
            ...
            set_error(::ets::process::detail::get_last_error(),
