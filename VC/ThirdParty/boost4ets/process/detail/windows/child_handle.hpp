...
struct child_handle
{
    ...
    child_handle(){memset(&proc_info, 0, sizeof(proc_info));}// = default;
    ~child_handle()
    {
        ::boost::detail::winapi::CloseHandle(proc_info.hProcess);
        ::boost::detail::winapi::CloseHandle(proc_info.hThread);
    }
private:child_handle(const child_handle & c);public:// = delete;
...
private:child_handle &operator=(const child_handle & c);public:// = delete;
