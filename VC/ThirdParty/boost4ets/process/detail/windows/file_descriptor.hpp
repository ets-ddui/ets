...
struct file_descriptor
{
    ...
    file_descriptor()// = default;
        : _handle(::boost::detail::winapi::INVALID_HANDLE_VALUE_)
    {
    ...
    {
        init(p.native().c_str(), mode);
    }

    file_descriptor(const std::string & path , mode_t mode = read_write) {init(path.c_str(), mode);}
        //: file_descriptor(path.c_str(), mode) {}
    file_descriptor(const std::wstring & path, mode_t mode = read_write) {init(path.c_str(), mode);}
        //: file_descriptor(path.c_str(), mode) {}

    file_descriptor(const char*    path, mode_t mode = read_write) {init(path, mode);}
private:
    void init(const char*    path, mode_t mode)
    {
        _handle = ::boost::detail::winapi::create_file(
                        path,
                        desired_access(mode),
...
public:
    file_descriptor(const wchar_t * path, mode_t mode = read_write) {init(path, mode);}
private:
    void init(const wchar_t * path, mode_t mode)
    {
        _handle = ::boost::detail::winapi::create_file(
                    path,
                    desired_access(mode),
...
private:file_descriptor(const file_descriptor & );public:// = delete;
    //file_descriptor(file_descriptor && ) = default;

private:file_descriptor& operator=(const file_descriptor & );public:// = delete;
