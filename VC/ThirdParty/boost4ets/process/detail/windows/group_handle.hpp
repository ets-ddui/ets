...
#include <process/detail/windows/job_workaround.hpp>
...
struct group_handle
{
    ...
    {
        _job_object = ::boost::detail::winapi::CreateJobObjectA(nullptr, nullptr);
        enable_break_away(_job_object);
    }
    ~group_handle()
    {
...
private:group_handle(const group_handle & c);public:// = delete;
    group_handle(group_handle && c) : _job_object(c._job_object)
    {
        c._job_object = ::boost::detail::winapi::invalid_handle_value;
    }
private:group_handle &operator=(const group_handle & c);public:// = delete;
    ...
    void add(handle_t proc, std::error_code & ec) BOOST_NOEXCEPT
    {
        if (!::boost::detail::winapi::AssignProcessToJobObject(_job_object, proc))
            ec = get_last_error();
    }
    ...
    bool has(handle_t proc, std::error_code & ec) BOOST_NOEXCEPT
    {
        ::boost::detail::winapi::BOOL_ is;
        if (!::boost::detail::winapi::IsProcessInJob(proc, _job_object,  &is))
            ec = get_last_error();
...
inline void terminate(const group_handle &p)
{
    if (!::boost::detail::winapi::TerminateJobObject(p.handle(), EXIT_FAILURE))
        ets::process::detail::throw_last_error("TerminateJobObject() failed");
}

inline void terminate(const group_handle &p, std::error_code &ec) BOOST_NOEXCEPT
{
    if (!::boost::detail::winapi::TerminateJobObject(p.handle(), EXIT_FAILURE))
