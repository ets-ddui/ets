...
#include <process/detail/traits/wchar_t.hpp>

#if defined( BOOST_WINDOWS_API )
#include <process/detail/windows/basic_cmd.hpp>
#include <process/detail/windows/cmd.hpp>
#elif defined( BOOST_POSIX_API )
#include <boost/process/detail/posix/basic_cmd.hpp>
#include <boost/process/detail/posix/cmd.hpp>
#endif

#include <process/shell.hpp>
...
{
    typedef exe_setter_<char> result_type;

    static exe_setter_<char> conv(const exe_setter_<wchar_t> & in)
    {
        return ::ets::process::detail::convert(in.exe_);
    }
};
...
{
    typedef exe_setter_<wchar_t> result_type;
...
struct arg_setter_
{
    ...
    arg_setter_(string_type & str)     : _args(1, str) {}
    arg_setter_(string_type && s)      : _args(1, std::move(s)) {}
    arg_setter_(const string_type & s) : _args(1, s) {}
    arg_setter_(const value_type* s)   : _args(1, std::move(s)) {}
...
{
    typedef arg_setter_<char, true> result_type;
    ...
    static arg_setter_<char, true> conv(const arg_setter_<wchar_t, true> & in)
    {
        ...
        std::transform(in._args.begin(), in._args.end(), vec.begin(),
                [](const std::wstring & ws) -> std::string
                {
                    return ::ets::process::detail::convert(ws);
                });
        return vec;
    }
};
...
{
    typedef arg_setter_<wchar_t, true> result_type;
...
{
    typedef arg_setter_<char, false> result_type;
...
{
    typedef arg_setter_<wchar_t, false> result_type;
...
struct exe_builder
{
    ...
    exe_builder(): not_cmd(false), shell(false) {}
