...
#include <process/detail/windows/search_path.hpp>
#endif
...
inline boost::filesystem::path search_path(const boost::filesystem::path &filename,
                                    const std::vector<boost::filesystem::path> path = ::ets::this_process::path())
{
    return ::ets::process::detail::api::search_path(filename, path);
}
}}
