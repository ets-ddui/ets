...
#include <process/detail/windows/start_dir.hpp>
#endif
...
{
    typedef api::start_dir_init<char> result_type;

    static api::start_dir_init<char> conv(const api::start_dir_init<wchar_t> & in)
    {
        return api::start_dir_init<char>(::ets::process::detail::convert(in.str()));
    }
};
...
{
    typedef api::start_dir_init<wchar_t> result_type;

    static api::start_dir_init<wchar_t> conv(const api::start_dir_init<char> & in)
    {
        return api::start_dir_init<wchar_t>(::ets::process::detail::convert(in.str()));
    }
};
 ...
 */
BOOST_CONSTEXPR_OR_CONST ::ets::process::detail::start_dir_ start_dir;
