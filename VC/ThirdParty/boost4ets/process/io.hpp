...
#include <process/detail/windows/asio_fwd.hpp>
#include <process/detail/windows/close_in.hpp>
#include <process/detail/windows/close_out.hpp>
#include <process/detail/windows/null_in.hpp>
#include <process/detail/windows/null_out.hpp>
#include <process/detail/windows/file_in.hpp>
#include <process/detail/windows/file_out.hpp>
#include <process/detail/windows/pipe_in.hpp>
#include <process/detail/windows/pipe_out.hpp>
#endif
...
template<typename T> struct is_streambuf    {
        typedef typename std::is_same<T, boost::asio::streambuf>::type type;
        static const typename type::value_type value = type::value;
};
template<typename T> struct is_const_buffer {
...
template<typename T> struct is_mutable_buffer {
        typedef typename std::integral_constant<bool,
...
struct null_t  {BOOST_CONSTEXPR null_t() {}};
struct close_t;

template<class>
struct std_in_
{
    BOOST_CONSTEXPR std_in_() {}
...
struct std_out_
{
    ...
    api::async_out_buffer<p1, p2, const boost::asio::mutable_buffer>     operator=(const boost::asio::mutable_buffer & buf)     const {return buf;}
    api::async_out_buffer<p1, p2, const boost::asio::mutable_buffers_1> operator=(const boost::asio::mutable_buffers_1 & buf) const {return buf;}
    api::async_out_buffer<p1, p2, boost::asio::streambuf>               operator=(boost::asio::streambuf & os)                   const {return os ;}
    ...
    template<int pin>/*, typename = typename std::enable_if<
            (((p1 == 1) && (pin == 2)) ||
             ((p1 == 2) && (pin == 1)))
             && (p2 == -1)>::type>*/
    BOOST_CONSTEXPR_OR_CONST std_out_<1, 2> operator& (const std_out_<pin>&) const
    {
        return std_out_<1, 2> ();
    }
...
BOOST_CONSTEXPR_OR_CONST ets::process::detail::close_t close;
///This constant is a utility to redirect streams to the null-device.
BOOST_CONSTEXPR_OR_CONST ets::process::detail::null_t  null;
...
BOOST_CONSTEXPR_OR_CONST ets::process::detail::std_in_<void>   std_in;
...
BOOST_CONSTEXPR_OR_CONST ets::process::detail::std_out_<1> std_out;
 ...
 */
BOOST_CONSTEXPR_OR_CONST ets::process::detail::std_out_<2> std_err;
