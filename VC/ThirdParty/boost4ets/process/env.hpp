...
#include <process/detail/windows/env_init.hpp>
#endif
...
struct env_set
{
    ...
    string_type key;
    string_type value;
    env_set(const string_type &p_sKey, const string_type &p_sValue)
        : key(p_sKey), value(p_sValue)
    {
    }
};
...
struct env_append
{
    ...
    string_type key;
    string_type value;
    env_append(const string_type &p_sKey, const string_type &p_sValue)
        : key(p_sKey), value(p_sValue)
    {
    }
};
...
struct env_reset
{
    typedef std::basic_string<Char> string_type;//using string_type = std::basic_string<Char>;
    string_type key;
    env_reset(const string_type &p_sKey)
        : key(p_sKey)
    {
    }
};
...
{
    typedef env_set<char> result_type;

    static env_set<char> conv(const env_set<wchar_t> & in)
    {
        return env_set<char>(::ets::process::detail::convert(in.key),
                ::ets::process::detail::convert(in.value));
    }
};
...
{
    typedef env_set<wchar_t> result_type;

    static env_set<wchar_t> conv(const env_set<char> & in)
    {
        return env_set<wchar_t>(::ets::process::detail::convert(in.key),
                ::ets::process::detail::convert(in.value));
    }
};
...
{
    typedef env_append<char> result_type;

    static env_append<char> conv(const env_append<wchar_t> & in)
    {
        return env_append<char>(::ets::process::detail::convert(in.key),
                ::ets::process::detail::convert(in.value));
    }
};
...
{
    typedef env_append<wchar_t> result_type;

    static env_append<wchar_t> conv(const env_append<char> & in)
    {
        return env_append<wchar_t>(::ets::process::detail::convert(in.key),
                ::ets::process::detail::convert(in.value));
    }
};
...
{
    typedef env_reset<char> result_type;

    static env_reset<char> conv(const env_reset<wchar_t> & in)
    {
        return env_reset<char>(::ets::process::detail::convert(in.key));
    }
};
...
{
    typedef env_reset<wchar_t> result_type;

    static env_reset<wchar_t> conv(const env_reset<char> & in)
    {
        return env_reset<wchar_t>(::ets::process::detail::convert(in.key));
    }
};
...
struct env_init
{
    ...
    env_init(const basic_environment<Char> &p_Env): env(p_Env) {}
};

template<>
struct char_converter<char, env_init<wchar_t>>
{
    typedef env_init<char> result_type;
...
{
    typedef env_init<wchar_t> result_type;
...
{
    typedef basic_environment<char> result_type;
...
{
    typedef basic_environment<wchar_t> result_type;
 ...
 */
BOOST_CONSTEXPR_OR_CONST ets::process::detail::env_ env;
